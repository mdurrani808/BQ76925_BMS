...
#define F(x) x
// Send Direct command
unsigned int directCommand(byte command) {
...
// Send Sub-command
void subCommand(unsigned int data) {
	i2c_hal.beginTransmission(BQ_I2C_ADDR_WRITE);
...
int16_t subCommandResponseInt(byte offset) {
	i2c_hal.beginTransmission(BQ_I2C_ADDR_WRITE);
...
void writeDataMemory(unsigned int addr, int16_t data,
		byte noOfBytes) {
	byte chksum = 0;
...
void writeDataMemory_WIthoutConfigUpdate(unsigned int addr, int16_t data,
		byte noOfBytes) {
	byte chksum = 0;
	chksum = calculateChecksum(chksum, LOW_BYTE(addr));
	chksum = calculateChecksum(chksum, HIGH_BYTE(addr));
...
unsigned int readDataMemory(unsigned int addr, int size) {
	i2c_hal.beginTransmission(BQ_I2C_ADDR_WRITE);
	i2c_hal.write(CMD_DIR_SUBCMD_LOW);
	i2c_hal.write(LOW_BYTE(addr));
	i2c_hal.write(HIGH_BYTE(addr));
...
byte calculateChecksum(byte oldChecksum, byte data) {
	if (!oldChecksum)
		oldChecksum = data;
	else
...
{
	byte alertReg;
	alertReg = getAlertStatusRegister();
	// Handle alarm here
	return alertReg;
}
...
void setCellOvervoltageProtection(unsigned int mv, unsigned int ms) {
...
void setCellUndervoltageProtection(unsigned int mv, unsigned int ms) {
	byte thresh = (byte) (mv / 50.6);
	uint16_t dly = (uint16_t) (ms / 3.3) - 2;
...
// in units of 2mV.
void setChargingOvercurrentProtection(unsigned int mv, byte ms) {
...
void setDischargingOvercurrentProtection(unsigned int mv, byte ms) {
	byte thresh = (byte) mv / 2;
	byte dly = (byte) (ms / 3.3) - 2;
...
// Set user-defined discharging current protection
void setDischargingShortcircuitProtection(bq76952_scd_thresh thresh,
		unsigned int us) {
...
void setChargingTemperatureMaxLimit(signed int temp, byte sec) {
	if (temp < -40 || temp > 120)
		temp = 55;
...
void setDischargingTemperatureMaxLimit(signed int temp, byte sec) {
	if (temp < -40 || temp > 120)
		temp = 60;
...
void setEnableRegulator(bool enable_reg1, bool enable_reg2) {
...
void setCHGFETProtectionsA(byte val) {
...
void bq76952_check_batt_status(void)
{
	...
	osDelay(50);
	getAllCellVoltages(cellArray);
	alarmStatus = getAlertStatusRegister();
	isDischarging = isDischarging();
	isCharging = isCharging();
	batt_status = getBatteryStatusRegister();
	...
	auto tmp_debug1 = readDataMemory(0x9236, 1); // read config
	uint8_t tmp_debug2 = readDataMemory(0x9286, 1); // read config

	unsigned int manufacturing_status = getManufacturingStatus(); // FET_EN: bit 4
	auto current_now = getCurrentNow();
	bq76952_temp_t tStat = getTemperatureStatus();
	auto temp=getInternalTemp();
	auto stackV = getStackVoltage();
	auto cellv=getCellVoltage(15);
	auto alert_raw = getAlertRawStatusRegister();
	bq76952_protection_t status = getProtectionStatus(); // safety status A register
	bq76952_safety_alert_c_t safety_alert_c = getSafetyAlert_C();
	// If any cell overvoltage was detected, read snapshot of all cell voltages
	if (status.bits.CELL_OV) {
		for (i = 0; i < 16; i++) {
		cov[i] = getCOVSnapshot(i);
		}
	}
